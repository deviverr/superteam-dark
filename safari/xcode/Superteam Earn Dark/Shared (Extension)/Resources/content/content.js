(() => {
  ...
  // itself. Hand it an absolute extension-origin URL once at startup
  // (chrome-extension:// on Chrome, moz-extension:// on Firefox)
  ...
  function setBundledAssetVars() {
    try {
      ROOT.style.setProperty(
        '--se-solana-logo',
        `url("${seApi.runtime.getURL('assets/solana-dark-logo.svg')}")`);
  ...
  function loadAndApply() {
    seApi.storage.local.get(null, stored => {
  ...
  /* AMO's linter and its reviewers flag every innerHTML assignment, even
     ones whose only input is a string literal a few lines up. The icons
     below are exactly that, so parse them explicitly instead: same result,
     and the source is left with no innerHTML for a reviewer to have to
     reason about.

     Every icon literal MUST carry xmlns="http://www.w3.org/2000/svg". The
     HTML parser infers the SVG namespace for a bare <svg>; the XML parser
     used here does not — it puts the element in the null namespace, where
     it is not an SVG element at all and paints nothing. The buttons still
     mount, still size, still click; they are just empty. Hence the
     namespace check below, which turns that silent blank into a visible
     failure at the one place it can be caught. */
  const SVG_NS = 'http://www.w3.org/2000/svg';
  const svgParser = new DOMParser();
  function svgNode(markup) {
    const root = svgParser.parseFromString(markup, 'image/svg+xml').documentElement;
    if (root.namespaceURI !== SVG_NS) {
      console.error('[EarnDark] icon markup is missing its xmlns; not an SVG node:', markup);
      return null;
    }
  ...
  function buildToolbar() {
    ...
    const menuGlyph = svgNode(themeIcon());
    if (menuGlyph) menuBtn.appendChild(menuGlyph);
    ...
    Object.entries(PRESETS).forEach(([key, p]) => {
      ...
      swatch.style.setProperty('--sw-bg', p.bg);
      swatch.style.setProperty('--sw-accent', p.accent);
  ...
  function renameSavedTheme(id) {
    ...
    seApi.storage.local.get(['custom.savedThemes'], stored => {
      ...
      seApi.storage.local.set({ 'custom.savedThemes': updated });
    });
  }
  ...
  function deleteSavedTheme(id) {
    ...
    seApi.storage.local.get(['custom.savedThemes'], stored => {
      const themes = stored['custom.savedThemes'] || [];
      seApi.storage.local.set({ 'custom.savedThemes': themes.filter(x => x.id !== id) });
    });
  }
  ...
  function updateToolbar() {
    ...
    document.querySelectorAll('.se-toolbar').forEach(cluster => {
      ...
      const glyph = toggleBtn && svgNode(on ? MOON_ICON : SUN_ICON);
      if (glyph) toggleBtn.replaceChildren(glyph);
    });
  ...
  function toggleDark() {
    ...
    seApi.storage.local.set({ mode: 'manual', darkEnabled: turningOn });
  }
  ...
  seApi.storage.onChanged.addListener((changes, area) => {
  ...
  seApi.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  /* ── Time-mode watcher (background-independent) ───────────────
     Time mode is driven by a chrome.alarms tick in the background,
     which is fine on desktop but not on iOS: the extension's
     background context is suspended whenever Safari isn't in the
     foreground, so the alarm that should have flipped the theme at
     21:00 simply never fires while the phone is locked, and the page
     stays light until something else pokes it. So the content script
     also re-evaluates on its own — once a minute while visible, and
     immediately on every return to the foreground, which is the case
     the alarm actually misses. Both are no-ops outside time mode, and
     decide() is idempotent, so this costs nothing on desktop. */
  function watchTimeMode() {
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible' && state.mode === 'time') loadAndApply();
  });
