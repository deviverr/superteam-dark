...
// See the note in service-worker.js: Safari strips `tab.url` from an
// unfiltered query, so every query here scopes by URL — using exactly the
// host_permissions patterns — instead of filtering the results by hand.
//
// An empty result therefore means "the active tab is not a themed Earn
// page", which is what the callers below already do the right thing with:
// the wallpaper probe and the theme broadcast have nobody to talk to, and
// the per-page override section hides itself.
const EARN_TABS        = { url: ['*://superteam.fun/earn*', '*://*.superteam.fun/earn*'] };
const ACTIVE_EARN_TABS = { ...EARN_TABS, active: true, currentWindow: true };

function notifyTabs() {
  seApi.tabs.query(EARN_TABS, tabs => {
    for (const t of tabs) {
      seApi.tabs.sendMessage(t.id, { type: 'SE_APPLY' }).catch(() => {});
    }
  });
}

function saveAndNotify(obj) {
  seApi.storage.local.set(obj)
    .then(() => notifyTabs())
...
function previewColor(cssVar, value) {
  seApi.tabs.query(ACTIVE_EARN_TABS, tabs => {
    for (const t of tabs) {
      seApi.tabs.sendMessage(t.id, { type: 'SE_PREVIEW_COLOR', key: cssVar, value }).catch(() => {});
    }
  });
}
...
seApi.storage.local.get(Object.values(KEYS), stored => {
  populateUI(stored);
  initPageOverrides(stored);
});

seApi.storage.local.get([KEYS.savedThemes], stored => {
...
function probeWallpaper(url) {
  ...
  seApi.tabs.query(ACTIVE_EARN_TABS, tabs => {
    const tab = tabs[0];
    if (!tab) return;
    seApi.tabs.sendMessage(tab.id, { type: 'SE_PROBE_WALLPAPER', url })
      .then(res => {
  ...
  themes.forEach(t => {
    ...
    rename.addEventListener('click', async e => {
      ...
      seApi.storage.local.get([KEYS.savedThemes], stored => {
        ...
        seApi.storage.local.set({ [KEYS.savedThemes]: all });
    ...
    del.addEventListener('click', async e => {
      ...
      seApi.storage.local.get([KEYS.savedThemes], stored => {
        ...
        seApi.storage.local.set({ [KEYS.savedThemes]: remaining });
...
$('reload-settings').addEventListener('click', () => {
  ...
  seApi.storage.local.get(Object.values(KEYS), stored => {
    ...
    seApi.tabs.query(ACTIVE_EARN_TABS, tabs => {
      const tab = tabs[0];
      if (tab) seApi.tabs.reload(tab.id);
    });
...
$('factory-reset').addEventListener('click', async () => {
  const ok = await askConfirm(
    ...
    'Reset');
  if (!ok) return;
  seApi.storage.local.clear(() => {
    seApi.storage.local.set(DEFAULTS, () => {
      populateUI(DEFAULTS);
      renderCustomThemes([]);
...
function savePageOverride() {
  if (!currentPageGroup) return;
  seApi.storage.local.get([KEYS.pageOverrides, ...HIDE_FIELDS.map(f => f[2])], stored => {
...
$('clear-page-override').addEventListener('click', () => {
  if (!currentPageGroup) return;
  seApi.storage.local.get([KEYS.pageOverrides], stored => {
    const all = stored[KEYS.pageOverrides] || {};
    delete all[currentPageGroup];
    saveAndNotify({ [KEYS.pageOverrides]: all });
    seApi.storage.local.get(HIDE_FIELDS.map(f => f[2]), globals => renderPageOverrides(all, globals));
  });
});
